...
int main() {
    //训练数据及对应标签
    float trainingData[8][2] = {{480, 500}, {50, 130},  {110, 32}, {490, 60},
                                {60, 190},  {200, 189}, {78, 256}, {45, 315}};
    ...
    Ptr<TrainData> trainData =
        TrainData::create(trainingDataMat, ROW_SAMPLE, labelsMat);
    ...
    for (int i = 0; i < sizeof(trainingData[0]); i++) {
        ...
        if (q == 1)
            circle(dst1, Point(trainingData[i][0], trainingData[i][1]), 5,
                   Scalar(0, 0, 255), -1, 8);
        else
            circle(dst1, Point(trainingData[i][0], trainingData[i][1]), 5,
                   Scalar(0, 255, 0), -1, 8);
